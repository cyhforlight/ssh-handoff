...
`shell-pty` session 会在新建命令 channel 后依次发送工作命令和 `exit`，并保持输入开启，直到远端主动结束 channel 或执行超时。
...
使用 `open --mode shell-pty` 创建此类 session。它的命令 channel 请求 PTY 并启动远端 Shell，随后输入预先给定的工作命令和 `exit`，并保持输入开启直到 Shell 会话结束或超时。这个 Shell 用于适配远端入口；ssh-handoff 不会在执行期间继续转发用户输入，因此工作命令仍须能够一次性、非交互地完成。需要持续输入、菜单操作或全屏终端的程序应在原始 Shell 中运行。
